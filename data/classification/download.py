...
def download_amazon_review(save_dir: str, langs: List[str] = ["en", "fr", "ja", "zh", "es", "de"]) -> None:
    for lang in langs:
        dataset = load_dataset(f"SetFit/amazon_reviews_multi_{lang}")
        dataset = dataset.rename_column("text", "sentence")
        to_csv(dataset, f"{save_dir}/MARC/{lang}")

        # construct binari classification (discard label 2, and use 0, 1 and 3, 4 as lables 0 and 1)
        for split, data in dataset.items():
            labels = np.array(data["label"])
            data = data.select(np.where(labels != 2)[0])
            data = data.map(lambda batch: {"label": [1 if x > 2 else 0 for x in batch["label"]]}, batched=True)
            dataset[split] = data
